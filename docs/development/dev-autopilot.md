...
> Historical implementation design. The active project feature directory is
> `trading/docs/features/`. Commands naming the former orchestration tool are
> retained only as history and are not a source of trading policy or runtime
> configuration.
...
Hermes cron (mỗi 30 phút)   ──►   Kiểm tra feature pipeline
                                      │
File-based state (trading/docs/features/)
                                      │
                                      ▼
...
in_progress rỗng → lấy P1 cao nhất từ pending.md
                 → tạo thư mục trading/docs/features/{name}/
                 → viết brief.md
...
Đọc trading/docs/features/{name}/brief.md

Nhiệm vụ: Design feature '{name}'
Output: trading/docs/features/{name}/design.md
...
```bash
echo "APPROVED" > trading/docs/features/{name}/APPROVED
```
...
```bash
opencode --model minimax-m3 --prompt "
Đọc trading/docs/features/{name}/design.md
...
Xem nhanh:
```bash
echo "=== PENDING ===" && cat trading/docs/features/pending.md
echo "=== IN PROGRESS ===" && cat trading/docs/features/in_progress.md
echo "=== DONE ===" && cat trading/docs/features/done.md
```
