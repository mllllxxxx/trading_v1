...
- `--kind` â€” how the tool is reached and probed. One of `cli`, `binary`, `mcp`,
...
- `--capability` â€” the workflow purpose a step looks the tool up by. Free-text
...
- `--scan` â€” for `mcp`/`skill`/`http`, a declarative path or URL that
...
`present` means **equipped** (config/file resolves), not **live this session** â€”
...
without penalty â€” this is what keeps the core seamless on a fresh install.
...
```
impact-analysis Â· deploy-verification Â· coverage Â· security-scan
performance-benchmark Â· documentation-lookup
```
