...
- **Testing Mandate (Strict)**: Implement comprehensive automated unit tests covering the new/modified logic.
...
- **Autopilot Autonomy**: Merge automatically once all tests pass 100%.
...
- **Testing Mandate (Strict)**: Write extensive automated tests (unit/integration) for all modified systems.
- Ask for human confirmation before implementation *only* if the overall direction remains ambiguous or conflicts with core rules; otherwise, proceed autonomously.
...
- **Autopilot Autonomy**: In fully autonomous mode, the agent is allowed to merge high-risk changes automatically *only* if all automated test suites pass 100%.
