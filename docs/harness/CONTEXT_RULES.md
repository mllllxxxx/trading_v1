...
| `docs/harness/GENERIC_ARCHITECTURE_TEMPLATE.md` | Skip | Should | Must |
...
| `docs/harness/GENERIC_ARCHITECTURE_TEMPLATE.md` | Skip | Should for code or boundary changes | Must |
| `docs/harness/TEST_MATRIX.md` or `scripts/bin/harness-cli query matrix` | Should | Must | Must |
...
| `docs/harness/GENERIC_ARCHITECTURE_TEMPLATE.md` | Skip | Should for structural changes | Must |
...
| --- | --- |
| Task touches trading logic, strategy constraints, or LLM prompt formatting | Read recent trade logs (`closed_trades.jsonl`), override logs (`llm_overrides.jsonl`), and prompt templates (`trading/auto/prompts.py`) to incorporate past failure lessons. |
...
| Task changes Harness policy, source hierarchy, risk classification, or validation requirements | Read `docs/harness/HARNESS.md`, `docs/harness/FEATURE_INTAKE.md`, `docs/harness/GENERIC_ARCHITECTURE_TEMPLATE.md`, and `docs/decisions/*`; pause if direction is ambiguous. |
