...
# Make /app importable so `from auto import journal` works when this module is
# loaded from site-packages (overridden by Dockerfile COPY in unified container).
_APP_DIR = "/app"
...
# ---------------------------------------------------------------------------
# Auto-trader JSON API. The HTML pages /trader and /trader/history are served
# by the React SPA (frontend/dist) so they get the Layout sidebar. Only the
...
async def get_trader_history(
    ...
    """Paginated closed-trade history with optional symbol + win/loss filters.
    ...
    try:
        ...
        closed.sort(key=lambda t: _parse_dt(t.get("closed_at", "")), reverse=True)

        if symbol:
            closed = [t for t in closed if t.get("symbol") == symbol]
        if result == "win":
            closed = [t for t in closed if float(t.get("pnl_usd", 0)) > 0]
        elif result == "loss":
            closed = [t for t in closed if float(t.get("pnl_usd", 0)) <= 0]
...
@app.get("/api/trader/history/stats")
async def get_trader_history_stats() -> dict:
    """Aggregate stats across all closed trades: winrate, P&L, durations, breakdowns."""
    try:
        from auto import journal as _journal  # type: ignore
        _journal.ensure_dirs()
        closed = _journal.read_closed_trades()
        ...
        rrs = [float(t.get("rr_ratio", 0)) for t in closed]
        ...
        for t in closed:
            ...
            ca = t.get("closed_at", "")
            try:
                dt1 = datetime.fromisoformat(oa.replace("Z", "+00:00"))
                dt2 = datetime.fromisoformat(ca.replace("Z", "+00:00"))
                durations.append(int((dt2 - dt1).total_seconds()))
            except (ValueError, TypeError):
                pass
        ...
        for t in closed:
            sym = t.get("symbol", "?")
            b = by_sym.setdefault(sym, {"wins": 0, "losses": 0, "pnl": 0.0, "n": 0})
        ...
        for t in closed:
            ...
            b = by_regime.setdefault(r, {"wins": 0, "losses": 0, "pnl": 0.0, "n": 0})
            pnl = float(t.get("pnl_usd", 0))
            b["wins" if pnl > 0 else "losses"] += 1
            b["pnl"] = round(b["pnl"] + pnl, 2)
            b["n"] += 1
...
async def get_trader_status() -> dict:
    """Forward to the auto-trader journal. Same data as auto/dashboard.py /api/status."""
    try:
        from auto import journal as _journal  # type: ignore
        _journal.ensure_dirs()
        ...
        payload = {"error": str(exc), "timestamp": datetime.now().isoformat()}
    return payload


@app.post("/api/trader/kill")
async def post_trader_kill() -> dict:
    """Touch /data/STOP to halt the auto-trader."""
    try:
        from auto import journal as _journal  # type: ignore
        _journal.KILL_SWITCH.touch()
        return {"ok": True, "msg": "kill switch set"}
    except Exception as exc:  # noqa: BLE001
...
# Severity mapping for alerts surfaced to the terminal.
# Lower priority = less urgent; UI sorts newest-first regardless.
_ALERT_SEVERITY: dict[str, str] = {
    # Critical — operator must act
    "kill_switch_active": "critical",
    "kill_switch": "critical",
    "h5_loss_streak_kill": "critical",
    "corrupt_journal": "critical",
    ...
    "bearish_confluence": "warning",
    "regime_error": "warning",
    "confluence_error": "warning",
    "confluence_missing_total_score": "warning",
    "confluence_bad_score": "warning",
    # Info — events the trader wants to see
    "max_open_positions": "info",
    "cooldown_cleared": "info",
    "fill": "info",
    "open": "info",
    "regime_change": "info",
    "auto_start": "info",
...
# When a "skip" event carries a critical reason, escalate severity.
_CRITICAL_SKIP_REASONS: set[str] = {
    "kill_switch_active",
    "h5_loss_streak_kill",
    "daily_loss_cap",
    "corrupt_journal",
...
def _alert_severity(entry: dict) -> str | None:
    """Resolve severity for one journal entry. Returns None to drop it."""
    t = entry.get("type", "")
    base = _ALERT_SEVERITY.get(t)
    if base is not None:
        return base
    if t == "skip":
        reason = entry.get("reason", "")
        if reason in _CRITICAL_SKIP_REASONS:
            return "critical"
        if reason in _WARNING_SKIP_REASONS:
            return "warning"
        return "info"
    return None


def _alert_message(d: dict) -> str:
    """Render an alert item's message from a decisions.jsonl entry."""
    t = d.get("type", "")
    sym = d.get("symbol")
    ...
    if t == "fill":
        pnl = d.get("pnl_usd")
        try:
            pnl_s = f"${float(pnl):+.2f}" if pnl is not None else "PnL ?"
        except (TypeError, ValueError):
            pnl_s = "PnL ?"
        reason = d.get("exit_reason", "exit")
        return f"{sym_str} closed · {pnl_s} · {reason}".strip()
    if t == "open":
        ...
        try:
            ...
            entry_s = "@ ?"
        return f"{sym_str} opened {side} {entry_s}".strip()
    if t == "regime_change":
        prev = d.get("previous") or d.get("from") or "?"
        new = d.get("regime") or d.get("to") or "?"
        return f"{sym_str} regime {prev} → {new}".strip()
    if t in ("kill_switch_active", "kill_switch", "h5_loss_streak_kill", "daily_loss_cap", "auto_stopped"):
        reason = d.get("reason", t)
        return f"TRADING HALTED — {reason}"
    if t == "cooldown_set":
        mins = d.get("minutes", "?")
        reason = d.get("reason", "")
        return f"cooldown {mins}m — {reason}".strip()
    if t == "cooldown_cleared":
        return "cooldown cleared"
    if t == "llm_cost_alert":
        pct = d.get("pct_of_cap")
        try:
            pct_s = f"{float(pct):.0f}%" if pct is not None else "?"
        except (TypeError, ValueError):
            pct_s = "?"
        return f"LLM cost cap reached {pct_s}"
    if t == "corrupt_journal":
        return f"corrupt journal: {d.get('error', 'unknown')}"
    if t in ("regime_error", "confluence_error"):
        return f"{t}: {d.get('error', 'unknown')}"
    if t == "weak_confluence":
        score = d.get("score")
        return f"weak confluence (score={score}) — skipped"
    if t == "bearish_confluence":
        score = d.get("score")
        return f"bearish confluence (score={score}) — skipped"
    if t == "max_open_positions":
        return "max open positions reached — skipped"
    if t == "skip":
        reason = d.get("reason", "unknown")
        sym_str = f" ({sym})" if sym else ""
        rem = d.get("remaining_s")
        rem_str = f" — {int(rem)}s left" if isinstance(rem, (int, float)) and rem > 0 else ""
        return f"skip{sym_str} — {reason}{rem_str}"
    return t or "event"


@app.get("/api/trader/alerts")
async def get_trader_alerts(limit: int = 100) -> dict:
    """Surface alert-worthy events from the decisions log.

    Reads the last `limit` decisions and maps each one to an alert with:
      - ts       — ISO 8601 timestamp from the journal entry
      - type     — original decision type
      - severity — critical | warning | info
      - message  — human-readable summary
      - symbol   — symbol if present in payload
      - payload  — original payload dict for drill-down
    """
    try:
        from auto import journal as _journal  # type: ignore
        _journal.ensure_dirs()
        if not _journal.DECISIONS_LOG.exists():
            return {"alerts": [], "count": 0, "ts": datetime.now().isoformat()}
        with _journal.DECISIONS_LOG.open("r", encoding="utf-8") as f:
            entries = [json.loads(line) for line in f if line.strip()][-max(1, min(limit, 1000)):]
        alerts: list[dict] = []
        for e in entries:
            sev = _alert_severity(e)
            if sev is None:
                continue
            payload = {k: v for k, v in e.items() if k not in ("ts", "type")}
            alerts.append({
                "ts": e.get("ts"),
                "type": e.get("type", ""),
                "severity": sev,
                "message": _alert_message(e),
                "symbol": e.get("symbol"),
                "payload": payload,
            })
        # Newest first.
        alerts.reverse()
        return {"alerts": alerts, "count": len(alerts), "ts": datetime.now().isoformat()}
    except Exception as exc:  # noqa: BLE001
        return {"alerts": [], "count": 0, "error": str(exc), "ts": datetime.now().isoformat()}
...
def _parse_feature_design(path: Path) -> dict | None:
    ...
    1. YAML front-matter between leading ``---`` fences::

        ---
        id: my-feature
        name: My Feature
        status: in-progress
        branch: dev-autopilot/my-feature
        updated: 2026-06-23
        ---
        # body...

    2. Bold-key format used by current features (no fences)::
    ...
    A sibling ``status`` file, if present, overrides the inline status.
    Returns id, name, status, branch, updated_at, summary — or None on failure.
    """
    try:
    ...
    for line in text.splitlines()[:40]:
        ...
        if not s.startswith("**") or ":**" not in s:
            continue
        try:
            key_part, _, val_part = s.partition(":**")
        except ValueError:
            continue
    ...
    if not meta.get("id") and not meta.get("name"):
        return None

    # Sibling status file wins over inline status (single source of truth).
    sibling_status = path.parent / "status"
    if sibling_status.exists():
        try:
            sibling_val = sibling_status.read_text(encoding="utf-8").strip()
            if sibling_val:
                meta["status"] = sibling_val
        except OSError:
            pass

    # First non-heading, non-metadata, non-empty body line becomes the summary.
    summary = ""
    ...
    return {
        ...
        "updated_at": meta.get("updated") or meta.get("updated_at") or "",
        "summary": summary,
    }
...
async def get_hermes_features() -> dict:
    ...
    for child in sorted(base.iterdir(), key=lambda p: p.name.lower()):
        ...
        if not design.exists():
            continue
        meta = _parse_feature_design(design)
        if meta is None:
            continue
        items.append(meta)
    return {"features": items, "count": len(items), "dir": str(base), "ts": datetime.now().isoformat()}
...
from berkshire_routes import register_berkshire_routes  # noqa: E402
