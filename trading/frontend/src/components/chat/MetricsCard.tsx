...
const SENTIMENT = {
  positive: "text-ttcc-green",
  neutral: "text-ttcc-text",
  negative: "text-ttcc-red",
} as const;
...
export const MetricsCard = memo(function MetricsCard({ metrics, compact = false }: Props) {
  ...
  return (
    <div className={cn(
      "grid gap-1.5 rounded-xl border border-ttcc-border-subtle/60 bg-ttcc-surface-2/20 p-3",
      compact ? "grid-cols-3" : "grid-cols-[repeat(auto-fit,minmax(120px,1fr))]"
    )}>
      {shown.map(({ k, v }) => (
        <div key={k} className="text-center py-1">
          <p className="text-[10px] text-ttcc-text-secondary uppercase tracking-wide font-medium">
            {getMetricLabel(k)}
          </p>
          <p className={cn(
