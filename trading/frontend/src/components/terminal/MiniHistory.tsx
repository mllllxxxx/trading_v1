...
export function MiniHistory({ trades }: { trades: MiniTrade[] }) {
  ...
  if (!rows.length) {
    return (
      <div className="font-mono text-[10px] text-ttcc-text-muted px-3 py-1">
      ...
      {rows.map((t, i) => {
        ...
        return (
          <div key={i} className="flex items-center gap-1.5 text-[11px] whitespace-nowrap">
            <span className="font-mono tabular text-[10px] text-ttcc-text-muted">
              {fmtTime(t.closed_at).substring(11, 16)}
            </span>
            <span className="font-mono font-semibold text-ttcc-text">
            ...
            <span
              className={
                up
                  ? "font-mono tabular text-ttcc-green font-semibold"
                  : "font-mono tabular text-ttcc-red font-semibold"
              }
            >
              {fmtUsd(t.pnl_usd)}
            </span>
            {i < rows.length - 1 ? (
              <span className="text-ttcc-text-muted/40 mx-1">·</span>
            ) : null}
          </div>
        );
      })}
